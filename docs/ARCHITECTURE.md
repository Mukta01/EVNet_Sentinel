...
To ensure cross-platform compatibility and seamless production deployment independent of the frontend, the entire backend (API and saved `.pkl` models) is packaged inside a **Docker Container** (Epic 3). This container can be deployed to any registry (e.g., AWS ECS, Render) allowing the frontend to make HTTP POST requests to the `/predict` endpoint.
