...
def train_and_predict_rf(X_train_path, y_train_path, X_test_path, predictions_dir, model_save_dir, plots_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, "rf_training_multi_distribution.png"))
    plt.close()
    ...
    model_multi = RandomForestClassifier(n_estimators=0, warm_start=True, max_depth=15, 
                                         class_weight=class_weight_multi, random_state=42, n_jobs=-1)
                                         
    with tqdm(total=n_estimators, desc="Training RF Multiclass (Trees)") as pbar:
        for i in range(step, n_estimators + step, step):
            model_multi.set_params(n_estimators=i)
    ...
    print("[*] Random Forest Training and Prediction Pipeline Complete!")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and Predict with Random Forest")
    ...
    train_and_predict_rf(args.input_X_train, args.input_y_train, args.input_X_test, 
